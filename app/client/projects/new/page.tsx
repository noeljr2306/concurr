...
import { Trash2, Plus, Lock, ShieldCheck, Layers } from "lucide-react";
...
export default function NewProjectPage() {
  ...
  function addMilestone() {
    setMilestones(ms => [...ms, { id:nextId, name:"", amount:"" }]);
  }
  function removeMilestone(id: number) {
    setMilestones(ms => ms.filter(m => m.id !== id));
  }
  function updateMilestone(id: number, field: "name"|"amount", val: string) {
    ...
    <div className="min-h-full bg-slate-100 flex flex-col pb-10">
      <main className="flex-1 flex flex-col items-center px-4 py-10">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 w-full max-w-[600px] p-10">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="flex items-center gap-2 text-[16px] font-bold text-slate-900">
                <Layers size={16} strokeWidth={2} className="text-indigo-700"/>
                Milestone Breakdown
              </h2>
              <span className="px-3 py-1 border border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider rounded-full">
                Draft Mode
              </span>
            </div>
                ...
                <div key={m.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-[13.5px] text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all"
                    />
                    ...
                    <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2.5 w-[110px]">
                      ...
                      <input
                        ...
                        type="number"
                        min="0"
                        className="w-full bg-transparent text-[13.5px] text-slate-800 focus:outline-none"
                      />
                    </div>
                    <button onClick={()=>removeMilestone(m.id)} className="text-slate-300 hover:text-red-400 transition-colors">
          ...
          >
            <Plus size={16} strokeWidth={2.5} className="rounded-full border-2 border-indigo-700"/>
            Add Another Milestone
          </button>

          {/* Total commitment box */}
          <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-6 py-5 mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-[22px] font-bold text-indigo-800">
                Total to Fund: ${total.toLocaleString(undefined,{minimumFractionDigits:0,maximumFractionDigits:0})}
              </p>
            </div>
        ...
        <div className="flex items-center justify-center gap-8 mt-8 text-[12.5px] text-slate-400">
          <span className="flex items-center gap-2">
            <ShieldCheck size={15} strokeWidth={1.8}/>256-bit SSL Secure
          </span>
          <span className="flex items-center gap-2">
            <Layers size={15} strokeWidth={1.8}/>Smart Contract Protected
          </span>
        </div>
