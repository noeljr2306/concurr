...
const navItems = [
  ...
  { label: "Escrows",    href: "/client/escrows",     icon: ShieldCheck     },
    ...
    <aside className="w-[220px] min-w-[220px] bg-white border-r border-slate-200 flex flex-col h-screen">
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-slate-100">
        <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            ...
            <rect x="13" y="2" width="9" height="9" rx="2" fill="#3b82f6" opacity="0.4" />
            <rect x="2" y="13" width="9" height="9" rx="2" fill="#3b82f6" opacity="0.4" />
      ...
      <div className="border-t border-slate-100 px-3 py-3 space-y-1">
        <div className="flex items-center gap-3 px-3 py-2.5">
          <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-[12px] font-bold flex-shrink-0">
            {session?.avatarInitials ?? "??"}
          </div>
          <div className="min-w-0">
            <p className="text-[13px] font-semibold text-slate-800 truncate">{session?.name ?? "Client"}</p>
